...
export class CssColorSupplier implements ColorSupplier {
  ...
  constructor(
    ...
    private fallbackColor?: Color | undefined
  ) {}
  ...
  get(): Color {
    ...
    this.cachedColor = this.resolveColorValue();
    return this.cachedColor;
  }

  private resolveColorValue(): Color {
    const colorValue = getComputedStyle(this.targetElement).getPropertyValue(this.variableName);

    if (colorValue == "") {
      console.error(`Missing color variable: ${this.variableName}`);
      return this.fallbackColor || COLOR_BLACK;
    }

    if (colorValue.startsWith('#')) {
      if (colorValue.length != 7) {
        console.error(`Invalid color variable hex color value: ${this.variableName}=${colorValue}`);
        return this.fallbackColor || COLOR_BLACK;
      }
      ...
      if (beginIndex < 0 || endIndex < 0) {
        console.error(`Invalid color variable rgb/rgba color value: ${this.variableName}=${colorValue}`);
        return this.fallbackColor || COLOR_BLACK;
      }
    ...
    } else if (colorValue in CssColorSupplier.namedColorToRgb) {
      return CssColorSupplier.namedColorToRgb[colorValue];
    } else {
      console.error(`Invalid/Unsupported color variable value: ${this.variableName}=${colorValue}`);
      return this.fallbackColor || COLOR_BLACK;
    }
  }

  // This property is kept at the bottom, due to its size
  private static namedColorToRgb: { [key: string]: Color } = {
